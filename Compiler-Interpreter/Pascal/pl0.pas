...
begin if cx > cxmax then
  ...
  if (x = opr) and (y = 0) and (z = 15) then outnr := outnr + 1;
  with code[cx] do
  begin f := x; l := y; a := z
  end;
  cx := cx + 1
...
procedure block(lev, tx: integer; fsys: symset);
  ...
  begin {statement}
    ...
    if sym = writesym then begin
      getsym; expression(fsys); gen(opr, 0, 15)
    end;
    test(fsys, [], 19)
  end {statement};
  ...
  repeat i := code[p]; p := p + 1;
    ...
    case f of
      ...
      opr: case a of  {operator}
             ...
             14: begin t := t + 1; write('> '); readln(s[t])
                 end;
      ...
      sto: begin s[base(l) + a] := s[t]; if outnr = 0 then writeln(s[t]); t := t - 1
           end;
