...
begin writeln(listfile, ' ****', ' ': cc - 1, '^', n: 2, ' ', message[n]: 60); err := err + 1
end {error};
...
procedure block(lev, tx: integer; fsys: symset);
  ...
  begin {statement}
    ...
    if sym = writesym then begin
      getsym; expression(fsys); gen(opr, 0, 15); out := out + 1
    end;
    test(fsys, [], 19)
  end {statement};
...
begin writeln(' START PL/0');
  ...
  repeat i := code[p]; p := p + 1;
    ...
    case f of
      ...
      sto: begin s[base(l) + a] := s[t]; if out = 0 then writeln(s[t]); t := t - 1
           end;
